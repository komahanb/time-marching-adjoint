...
contains
  ...
  subroutine evaluate_spatial_error(npts, rmse)
    ...
    use crank_nicolson_integrator_class       , only : CNI
    ...
    ! implicit euler
    bdfobj = BDF(system = system, tinit=10.0d0, tfinal = 11.0d0, &
         & h=1.0d-2, implicit = .true., accuracy_order=1)
    call bdfobj % to_string()
    call bdfobj % solve()
    rmse(4) = 0.0d0
    do j = 1, system % get_num_state_vars()
       do k = 2,2!1, bdfobj % num_time_steps          
          t = bdfobj % time((bdfobj % num_stages+1)*k - bdfobj % num_stages)
          x = system % X(1,j)
          error = bdfobj % U ((bdfobj % num_stages+1)*k - bdfobj % num_stages, 1, j) - exact(t, x)
    ...
    rmse(5) = 0.0d0
     do j = 1, system % get_num_state_vars()
        do k = 2,2!1, bdfobj % num_time_steps          
           t = bdfobj % time((bdfobj % num_stages+1)*k - bdfobj % num_stages)
           x = system % X(1,j)
           !print *, t, x, bdfobj % U(1,1,j), exact(t,x), (bdfobj % num_stages+1)*k - bdfobj % num_stages
           error = bdfobj % U ((bdfobj % num_stages+1)*k - bdfobj % num_stages, 1, j) - exact(t, x)
    ...
    ! cni
    cniobj = CNI(system = system, tinit=10.0d0, tfinal = 11.0d0, &
         & h=1.0d-2)
    call cniobj % to_string()
    call cniobj % solve()
    rmse(6) = 0.0d0
    do j = 1, system % get_num_state_vars()
       do k = 2,2 !1, cniobj % num_time_steps          
          t = cniobj % time((cniobj % num_stages+1)*k - cniobj % num_stages)
          x = system % X(1,j)
          error = cniobj % U ((cniobj % num_stages+1)*k - cniobj % num_stages, 1, j) - exact(t, x)
...
!!$    abmobj = ABM(system = test_system, tinit=10.0d0, tfinal = 40.0d0, &
!!$         & h=1.0d-2, implicit=.false., accuracy_order=1)
!!$    call abmobj % to_string()
!!$    call abmobj % solve()
!!$    call abmobj % write_solution(trim(prefix)//"-transport-implicit-abm1.dat")
...
!!$    call bdfobj % write_solution(trim(prefix)//"-transport-explicit-euler.dat")   
    ...
    cniobj = CNI(system = test_system, tinit=10.0d0, tfinal = 40.0d0, &
         & h=1.0d-2)
    call cniobj % to_string()
    call cniobj % solve()
    call cniobj % write_solution(trim(prefix)//"-transport-cni.dat")
